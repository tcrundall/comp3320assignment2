...
static void idleTime (void)		// when idle...
{
  ...
  if(paused) {
    snprintf(progress, sizeof(progress),
    	"Nodes %d Separation %5.2f  Ball %4.2f Offset %4.2f",
    	n, sep, rball, offset);
   ...
   /* YOU NEED TO WRITE THIS ROUTINE IN MYCODE.C      */
    ...
    loopcode(n,mass,fcon,delta,grav,sep,rball,xball,yball,zball,
	     dt,x,y,z,fx,fy,fz,vx,vy,vz,oldfx,oldfy,oldfz,&pe,&ke,&te);

   /***************E N D*******************************/
    if(loop%update==0){
      gettimeofday(tp2, NULL);
      elapsed = (float)(tp2->tv_sec-tp1->tv_sec)+ (float)(tp2->tv_usec-tp1->tv_usec)*1.0e-6;
      snprintf(progress, sizeof(progress),
...
static void init(void)
{
  ...
  if(ballrendersize < 0.01)ballrendersize = rball;
...
void mouse(int button, int state, int x, int y){		// when button is pressed
...
void keyboard(unsigned char key, int x, int y)		// key bindings
{
  ...
  case 32: //space
    ...
    break;
  case 'b':
    if (rball > 1) rball -= 0.25;
    reset_simulation();
    break;
  case 'B':
    if (rball < 5) rball += 0.25;
    reset_simulation();
    break;
  case '[':
    if (offset < rball + 1) offset += 0.25;
    reset_simulation();
    break;
  case ']':
